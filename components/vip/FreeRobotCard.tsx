...
const FreeRobotCard = ({ id, name, description = "", features = [], imageSrc, downloadUrl = "#", category = "Automated" }: FreeRobotCardProps) => {
    ...
    const handleDownload = (e: React.MouseEvent) => {
        ...
        setTimeout(() => {
            ...
            // We set isDownloading false after a delay or on unmount, but since we're navigating, unmount will handle it.
            // However, to show the text change immediately:
        }, 300);
    };
    return (
        <div
        ...
        >
            ...
            <div className="relative aspect-square sm:aspect-[16/10] overflow-hidden bg-gradient-to-b from-[#f0fdf9] to-white">
                ...
                />
                <div className="absolute top-2 left-2 md:top-4 md:left-4 bg-[#1e293b]/90 backdrop-blur-sm px-2 md:px-4 py-1 md:py-2 rounded-lg border border-white/10 shadow-lg hidden sm:block max-w-[80%]">
                    <h3 className="text-white text-xs md:text-base font-bold tracking-tight truncate">{name}</h3>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-auto pt-2">
                    ...
                    <button
                        ...
                        className={`w-full sm:w-auto flex items-center justify-center gap-1.5 md:gap-2 font-bold py-3 md:py-4 px-4 md:px-8 rounded-xl shadow-lg transition-all duration-300 active:scale-95 text-xs md:text-base uppercase tracking-tight ${isLoggedIn
                            ? "bg-gradient-to-r from-[#059669] to-[#10b981] hover:from-[#10b981] hover:to-[#34d399] text-white"
